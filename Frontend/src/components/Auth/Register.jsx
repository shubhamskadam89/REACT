...
export default function Register() {
  ...
  const vehicleRegRegex = /^[A-Z]{2}\d{2}[A-Z]{2}\d{4}$/i;
    ...
    switch (name) {
      ...
      case 'vehicleRegNumber':
        if (form.role === 'AMBULANCE_DRIVER' || form.role === 'FIRE_DRIVER') {
          ...
          else if (!vehicleRegRegex.test(value)) error = 'Enter valid Indian Reg (e.g., MH12AB1234).';
        }
        break;
      case 'hospitalID':
  ...
  const validateAllFields = () => {
    ...
    // Admin roles don't need additional fields beyond the basic ones
  ...
  const handleSubmit = async (e) => {
    ...
    switch (form.role) {
      ...
      case 'POLICE_OFFICER':
        ...
        break;
      default:
        // All other roles (USER, ADMIN, FIRE_STATION_ADMIN, AMBULANCE_ADMIN, POLICE_STATION_ADMIN) 
    ...
    // Admin roles (USER, ADMIN, FIRE_STATION_ADMIN, AMBULANCE_ADMIN, POLICE_STATION_ADMIN) don't need driver-specific fields
    if (['USER', 'ADMIN', 'FIRE_STATION_ADMIN', 'AMBULANCE_ADMIN', 'POLICE_STATION_ADMIN'].includes(form.role)) { 
